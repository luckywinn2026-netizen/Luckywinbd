/**
 * Build Admin release APK only (bd.luckywin.admin – admin interface only).
 * Run: npm run cap:apk:release:admin
 * Output: dist/apk/lucky-win-admin-release.apk
 */
...
console.log('1. Building Admin variant (VITE_APP_VARIANT=admin)...');
...
console.log('3. Building Admin APK...');
execSync(`${gradlew} assembleAdminRelease`, { cwd: androidDir, stdio: 'inherit', env });

const apkPath = path.join(androidDir, 'app', 'build', 'outputs', 'apk', 'admin', 'release', 'app-admin-release.apk');
if (!fs.existsSync(distDir)) fs.mkdirSync(distDir, { recursive: true });
const destPath = path.join(distDir, 'lucky-win-admin-release.apk');
