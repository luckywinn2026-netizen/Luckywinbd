...
import { BookOfDeadSound } from './BookOfDeadSoundEngine';

// ─── Egyptian Symbols ───
const SYMBOLS = ['📖', '🏺', '⚱️', '🗿', '💀', '🔮', '🐍', '🪲', '👁️', '🌙'];

const SYMBOL_WEIGHTS: Record<number, number> = {
  0: 5, 1: 8, 2: 8, 3: 7, 4: 6, 5: 7, 6: 10, 7: 12, 8: 12, 9: 12,
};
...
const getTriplePayout = (idx: number): number => {
  if (idx === 0) return 100 + Math.floor(Math.random() * 401); // 📖 Book
  if (idx === 4) return 50 + Math.floor(Math.random() * 201);  // 💀 Skull
  if (idx === 5) return 30 + Math.floor(Math.random() * 71);   // 🔮 Crystal
  if (idx === 3) return 20 + Math.floor(Math.random() * 31);   // 🗿 Pharaoh
  if (idx === 1) return 10 + Math.floor(Math.random() * 21);   // 🏺 Vase
  if (idx === 2) return 10 + Math.floor(Math.random() * 21);   // ⚱️ Urn
  if (idx === 6) return 5 + Math.floor(Math.random() * 11);    // 🐍 Snake
  if (idx === 7) return 3 + Math.floor(Math.random() * 5);     // 🪲 Scarab
  if (idx === 8) return 2; // 👁️
  if (idx === 9) return 2; // 🌙
  return 1;
};
...
const CylinderReel = ({ finalIdx, spinning, reelIndex, onStop, soundEnabled, isWin, spinId }: {
  ...
  useEffect(() => {
    ...
    const animate = (now: number) => {
      ...
      if (soundEnabled) {
        ...
        if (curFace !== prevFace) BookOfDeadSound.tick();
      }
      if (progress < 1) { animRef.current = requestAnimationFrame(animate); }
      else {
        currentAngleRef.current = -(finalIdx * FACE_ANGLE);
        setRotationX(-(finalIdx * FACE_ANGLE));
        if (soundEnabled) BookOfDeadSound.reelStop(reelIndex);
        onStop();
      }
    };
    animRef.current = requestAnimationFrame(animate);
  ...
  const faces = SYMBOLS.map((symbol, i) => (
    <div key={i} className="absolute w-full flex items-center justify-center"
      ...
      <span className="select-none" style={{ fontSize: 36, filter: i === finalIdx && !spinning && isWin ? 'drop-shadow(0 0 25px #ffd700)' : 'none' }}>{symbol}</span>
    </div>
  ));
  ...
  return (
    <div className="relative flex-shrink-0" style={{ width: 100, height: containerH }}>
      <div className="absolute inset-0 overflow-hidden" style={{
        borderRadius: '50% / 14%',
        background: 'linear-gradient(180deg, #1a0a00 0%, #3a1c00 6%, #6b4000 12%, #d4b896 16%, #e8d8c0 25%, #f4ece0 40%, #faf6f0 50%, #f4ece0 60%, #e8d8c0 75%, #d4b896 84%, #6b4000 88%, #3a1c00 94%, #1a0a00 100%)',
        border: '3px solid #c9a030', boxShadow: '0 0 0 2px #7a5a10, 0 8px 30px rgba(0,0,0,0.8), inset 0 0 20px rgba(255,255,255,0.2)',
      }}>
        ...
        <div className="absolute top-0 left-0 right-0 h-[30px] z-10" style={{ background: 'linear-gradient(180deg, #0d0500, #2a1200, #5a3000, rgba(90,48,0,0))', borderBottom: '2px solid #c9a030', borderRadius: '48% 48% 0 0 / 100% 100% 0 0' }} />
        <div className="absolute bottom-0 left-0 right-0 h-[30px] z-10" style={{ background: 'linear-gradient(0deg, #0d0500, #2a1200, #5a3000, rgba(90,48,0,0))', borderTop: '2px solid #c9a030', borderRadius: '0 0 48% 48% / 0 0 100% 100%' }} />
        ...
        <div className="absolute left-0 right-0 top-[31px] h-[2px] z-10" style={{ background: 'linear-gradient(90deg, transparent 5%, #8B6914, #ffd700, #ffe066, #ffd700, #8B6914, transparent 95%)' }} />
        <div className="absolute left-0 right-0 bottom-[31px] h-[2px] z-10" style={{ background: 'linear-gradient(90deg, transparent 5%, #8B6914, #ffd700, #ffe066, #ffd700, #8B6914, transparent 95%)' }} />
      </div>
      {isWin && !spinning && (
...
const BookOfDeadGame = () => {
  ...
  useActivePlayer('book-of-dead', 'Lucky Book of Dead', 'slot', betAmount);
  ...
  useEffect(() => {
    ...
    if (d1 === d2 && d2 === d3) {
      ...
      if (oc.outcome === 'mega_win') { setShowBigWin(true); setShowConfetti(true); if (soundEnabled) BookOfDeadSound.jackpot(); setTimeout(() => { setShowBigWin(false); setShowConfetti(false); }, 800); }
      else if (oc.outcome === 'big_win') { setShowBigWin(true); setShowConfetti(true); if (soundEnabled) BookOfDeadSound.bigWin(); setTimeout(() => { setShowBigWin(false); setShowConfetti(false); }, 600); }
      else { setShowConfetti(true); if (soundEnabled) BookOfDeadSound.win(); setTimeout(() => setShowConfetti(false), 500); }
      addWin(winAmt, 'Book of Dead', 'slot', mult, betAmount, 'book-of-dead');
      setSpinHistory(prev => [{ win: true, amount: winAmt }, ...prev.slice(0, 14)]);
    } else {
      setLastWin(0); setWinMult(0); setIsWin(false);
      if (soundEnabled) BookOfDeadSound.loss();
      logLoss(betAmount, 'Book of Dead', 'slot', 'book-of-dead');
  ...
  const spin = async () => {
    ...
    if (!placeBet(betAmount, 'Book of Dead', 'slot')) return;
    setLastWin(0); setWinMult(0); setIsWin(false); setSpinning(true); setStoppedReels(0);
    if (soundEnabled) BookOfDeadSound.spin();

    let outcome: { outcome: string; maxWinAmount: number } = { outcome: 'loss', maxWinAmount: 0 };
    try {
      const data = await api.gameOutcome({ bet_amount: betAmount, game_type: 'slot', game_id: 'book-of-dead' });
    ...
    if (outcome.outcome === 'loss') {
      ...
      const s = [0, 4][Math.floor(Math.random() * 2)]; finalIdxs = [s, s, s];
    } else if (outcome.outcome === 'big_win') {
      const s = [0, 4, 5, 3][Math.floor(Math.random() * 4)]; finalIdxs = [s, s, s];
    } else {
  ...
  const adjustBet = (dir: number) => {
    ...
    if (soundEnabled) BookOfDeadSound.buttonClick();
  };

  return (
    <AuthGate>
      <GameLoadingScreen show={showSplash} gameName="📖 Lucky Book of Dead" onComplete={handleLoadingComplete} />
      <div className="min-h-screen flex flex-col" style={{ background: 'linear-gradient(180deg, #1a0a00 0%, #2a1500 30%, #3a2000 60%, #1a0a00 100%)' }}>
        {/* Header */}
        <div className="flex items-center gap-3 p-3 pt-2">
          ...
          <h1 className="font-heading font-bold text-lg" style={{ color: '#ffd700', textShadow: '0 0 10px rgba(255,215,0,0.5)' }}>📖 Lucky Book of Dead</h1>
          <div className="ml-auto flex items-center gap-2">
            <PaytableModal gameName="Lucky Book of Dead" betAmount={betAmount} {...SIMPLE_3REEL_PAYTABLE} symbols={SYMBOLS.map(s => ({ label: s }))} />
            <button onClick={() => setSoundEnabled(!soundEnabled)} className="p-2">{soundEnabled ? <Volume2 size={18} className="text-amber-200" /> : <VolumeX size={18} className="text-amber-200/50" />}</button>
            <div className="rounded-full px-3 py-1" style={{ border: '2px solid #c9a030', background: 'rgba(201,160,48,0.15)' }}>
              <span className="text-sm font-heading font-bold text-amber-200">৳{balance.toLocaleString()}</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 mx-3 mb-2 relative">
          ...
          <div className="relative rounded-2xl p-[3px] animate-pulse-glow" style={{
            background: 'linear-gradient(180deg, #ffe066, #c9a030, #ffd700, #8B6914, #ffe066)',
          }}>
            {/* Inner dark frame border */}
            <div className="rounded-[13px] p-[2px]" style={{ background: '#1a0a00' }}>
              {/* LED strip ring */}
              <div className="rounded-xl p-[2px] relative overflow-hidden" style={{
                background: 'linear-gradient(90deg, #ff4400, #ffd700, #44cc00, #00aaff, #cc44ff, #ff4400)',
                ...
                <div className="rounded-[9px] relative overflow-hidden flex flex-col items-center justify-center min-h-[280px]" style={{
                  background: 'linear-gradient(180deg, #2a1500, #1e0e00, #0d0600)',
                }}>
                  {/* Top decorative arch */}
                  <div className="absolute top-0 left-0 right-0 h-8 flex items-center justify-center" style={{
                    background: 'linear-gradient(180deg, rgba(201,160,48,0.12), transparent)',
                  }}>
                    <div className="flex gap-2">
                      {[0,1,2,3,4,5,6].map(i => (
                        <div key={i} className="w-1.5 h-1.5 rounded-full" style={{
                          background: `hsl(${30 + i * 8} 85% 55%)`,
                          boxShadow: `0 0 4px hsl(${30 + i * 8} 85% 55%)`,
                  ...
                  <div className="absolute top-3 left-1/2 -translate-x-1/2 text-3xl opacity-30">🏛️</div>

                  {/* Reel Window */}
                  <div className="mt-6 mb-4 relative">
                    <div className="flex gap-2 p-3 rounded-xl relative" style={{
                      background: 'linear-gradient(180deg, #0d0500, #1a0a00, #0d0500)',
                      boxShadow: 'inset 0 4px 12px rgba(0,0,0,0.6), inset 0 -4px 12px rgba(0,0,0,0.4), 0 0 20px rgba(201,160,48,0.08)',
                      ...
                      <div className="absolute left-0 right-0 top-1/2 -translate-y-1/2 h-[2px] z-10 pointer-events-none" style={{
                        background: 'linear-gradient(90deg, transparent, rgba(255,215,0,0.6), #ffd700, rgba(255,215,0,0.6), transparent)',
                  ...
                  <AnimatePresence>
                    {lastWin > 0 && !spinning && (
                      <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} exit={{ scale: 0 }} className="text-center mb-2">
                        ...
                        <p className="font-heading font-bold text-xl text-amber-200">৳{lastWin.toLocaleString()} ({winMult}x)</p>
                      </motion.div>
                    )}
                  </AnimatePresence>
        ...
        <SlotControlPanel
          ...
          onToggleAuto={() => { setAutoSpin(!autoSpin); if (soundEnabled) BookOfDeadSound.buttonClick(); }}
          onToggleTurbo={() => { setTurboMode(!turboMode); if (soundEnabled) BookOfDeadSound.buttonClick(); }}
          accentColor="ffd700"
          spinEmoji="📖"
        />
      </div>
