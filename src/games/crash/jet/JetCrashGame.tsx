...
// JET THEME: Dark emerald green & lime/neon green
const THEME = {
  name: 'Lucky Jet Crash',
  emoji: '🛩️',
  channelName: 'jet-live',
  gameId: 'jet',
  curveColor: '#22c55e',
  curveGlow: 'rgba(34, 197, 94, 0.4)',
  gradientStart: 'rgba(34, 197, 94, 0.05)',
  gradientMid: 'rgba(34, 197, 94, 0.2)',
  gradientEnd: 'rgba(34, 197, 94, 0.4)',
  crashColor: '#ef4444',
  gridColor: 'rgba(34, 197, 94, 0.06)',
  titleClass: 'text-emerald-400',
  balanceBorder: 'border border-emerald-500/30',
  balanceText: 'text-emerald-400',
  canvasBorder: 'border border-emerald-500/30',
  cardBorder: 'border border-emerald-500/20',
  countdownColor: 'text-emerald-400',
  betBtnClass: 'bg-gradient-to-r from-emerald-500 to-green-600 text-white',
  cashoutBtnClass: 'bg-gradient-to-r from-yellow-500 to-amber-600 text-white',
  historyHighClass: 'bg-emerald-500/15 text-emerald-400',
  tabActiveClass: 'bg-emerald-500/20 text-emerald-300',
  multiplierColor: (m: number) => m >= 5 ? 'text-yellow-400' : m >= 2 ? 'text-emerald-400' : 'text-foreground',
  bgGradient: 'bg-gradient-to-b from-[hsl(160,50%,10%)] to-[hsl(160,60%,5%)]',
  particleColor1: 'hsl(142, 76%, 46%)',
  particleColor2: 'hsl(160, 60%, 40%)',
};
...
const JetCrashGame = () => {
  ...
  const prevJetStateRef = useRef<string>('waiting');
  useEffect(() => {
    if (prevJetStateRef.current === gameState) return;
    const prev = prevJetStateRef.current;
    prevJetStateRef.current = gameState;
      ...
      <div className={`h-screen ${THEME.bgGradient} flex flex-col relative overflow-hidden`}>
        ...
        <div className="relative z-20 px-3 mb-2">
          <div className="flex items-center gap-1.5">
            ...
            <button onClick={() => setHistoryOpen(!historyOpen)} className="flex-shrink-0 p-1.5 bg-secondary rounded-lg hover:bg-emerald-500/20 transition-colors">
        ...
        <div className={`relative z-10 mx-3 rounded-xl overflow-hidden bg-background ${THEME.canvasBorder}`} style={{ minHeight: 200 }}>
          ...
          <style>{`@keyframes skyFlyLeft { 0% { transform: translateX(calc(100% + 50px)); left: 100%; } 100% { transform: translateX(calc(-100% - 50px)); left: 0%; } }`}</style>
          ...
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-[3]">
            {gameState === 'waiting' && (
              <div className="text-center">
                ...
                <p className={`${THEME.countdownColor} text-xs mt-1 animate-pulse font-bold`}>Place your bets!</p>
              </div>
            )}
            {gameState === 'flying' && (
            ...
            <div key={idx} className={`bg-card rounded-xl p-3 ${THEME.cardBorder}`}>
              ...
              <div className="flex items-center gap-2">
                <div className="flex items-center bg-background rounded-lg flex-1">
                  <button onClick={() => updatePanel(idx, { stake: String(Math.max(10, Number(panels[idx].stake) - 10).toFixed(2)) })} disabled={panels[idx].hasBet} className="px-2 py-2 text-muted-foreground text-lg font-bold disabled:opacity-30">−</button>
              ...
              <div className="flex gap-1.5 mt-2">
                {[5, 50, 100, 500].map(v => (
                  <button key={v} onClick={() => updatePanel(idx, { stake: String(v.toFixed(2)) })} disabled={panels[idx].hasBet}
                    className="flex-1 py-1 text-[11px] font-bold bg-secondary hover:bg-emerald-500/30 hover:text-emerald-300 text-muted-foreground rounded transition-colors disabled:opacity-30">{v}</button>
                ))}
              </div>
          ...
          <div className="mt-3">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-xs text-muted-foreground"><span className="text-emerald-400 font-bold">{onlineCount.toLocaleString()}</span> online</span>
              </div>
            </div>
            <div className="flex gap-0 mb-2">
                ...
                <>
                  <div className="flex gap-1 mb-2">
                    {(['today', 'week', 'month'] as const).map(st => (
                      <button key={st} onClick={() => setTopSubTab(st)} className={`flex-1 py-1 text-[10px] font-bold rounded-md ${
                        topSubTab === st ? 'bg-emerald-500 text-white' : 'bg-secondary text-muted-foreground'
                  ...
                  <div className="max-h-[300px] overflow-y-auto">
                    {(topSubTab === 'today' ? getTodayTopPlayers() : topSubTab === 'week' ? getWeekTopPlayers() : getMonthTopPlayers()).map((p, i) => (
                      <div key={p.id} className={`flex items-center py-1.5 px-1 text-xs border-b border-border last:border-0 ${i < 3 ? 'bg-emerald-500/5' : ''}`}>
                        <span className={`w-5 font-bold ${i < 3 ? 'text-emerald-400' : 'text-muted-foreground'}`}>{i === 0 ? '🥇' : i === 1 ? '🥈' : i === 2 ? '🥉' : `${i + 1}`}</span>
                        <span className="flex-1 truncate text-foreground font-medium">{p.username}</span>
                        <span className="w-16 text-right text-emerald-400 font-bold">{p.maxMultiplier.toFixed(2)}x</span>
                ...
                <>
                  <div className="flex items-center justify-between mb-2 pb-1 border-b border-border">
                    ...
                    <div className="flex items-center gap-3">
                      <span className="text-[10px] text-muted-foreground">👥 <span className="text-emerald-400 font-bold">{totalRoundPlayers.toLocaleString()}</span></span>
                    ...
                    {displayedRoundBets.map((b) => (
                      <div key={b.id} className={`flex items-center py-1.5 px-1 text-xs border-b border-border last:border-0 ${
                        ...
                        <div className="flex items-center gap-1.5 flex-1 min-w-0">
                          <div className={`w-5 h-5 rounded-full flex items-center justify-center text-[8px] text-foreground ${b.isReal ? 'bg-emerald-500' : 'bg-secondary'}`}>{b.isReal ? '⭐' : '👤'}</div>
                          <span className={`truncate max-w-[80px] ${b.isReal ? 'text-emerald-400 font-bold' : 'text-muted-foreground'}`}>{b.username}</span>
                        </div>
                        <span className="w-16 text-right text-muted-foreground">৳{b.bet_amount.toLocaleString()}</span>
                        <span className={`w-14 text-right font-bold ${b.status === 'cashed_out' ? 'text-emerald-400' : b.status === 'lost' ? 'text-destructive' : 'text-muted-foreground'}`}>
